// ==WindhawkMod==
// @id              clipboard-to-file
// @name            Paste Clipboard Content to File
// @description     Paste text and images from clipboard as files in Explorer, file dialogs, and Total Commander
// @version         1.5
// @author          BSODCode (original version by Anixx)
// @github          https://github.com/BSODCode
// @include         *
...
on the desktop, in open/save file dialogs, and in Total Commander.
...
- Works in Total Commander panels
...
// ============================================================
static bool IsTCWindow(HWND hwnd)
{
    HWND root = GetAncestor(hwnd, GA_ROOT);
    if (!root) return false;
    WCHAR cls[64] = {};
    GetClassNameW(root, cls, 64);
    return (wcscmp(cls, L"TTOTAL_CMD") == 0);
}

static bool GetTCCurrentPath(std::wstring& path)
{
    WCHAR buf[MAX_PATH] = {};
...
{
    if (IsTCWindow(hwnd)) return GetTCCurrentPath(path);
...
static bool CanPasteInWindow(HWND hwnd)
{
    ...
    bool isShell = IsShellViewWindow(hwnd);
    bool isTC = IsTCWindow(hwnd);
    if (!isShell && !isTC)                     return false;
    
    if (isTC)
    {
        WCHAR cls[64] = {};
        GetClassNameW(hwnd, cls, 64);
        if (wcsicmp(cls, L"Edit") == 0 || wcsicmp(cls, L"TEdit") == 0)
            return false;
    }
...
// ============================================================
//  Target window at cursor
// ============================================================
static HWND GetTargetWindowAtCursor()
{
    ...
    while (hwnd)
    {
        if (IsShellViewWindow(hwnd) || IsTCWindow(hwnd)) return hwnd;
...
static bool PerformPaste(HWND sourceWindow)
{
    if (!IsShellViewWindow(sourceWindow) && !IsTCWindow(sourceWindow)) return false;
    if (!HasNonFileClipboardContent())      return false;
    ...
    if (errCode == 0)
    {
        ...
        if (IsTCWindow(sourceWindow))
        {
            HWND root = GetAncestor(sourceWindow, GA_ROOT);
            PostMessageW(root, WM_USER + 51, 540, 0); 
        }
        return true;
    }
...
BOOL WINAPI EnableMenuItem_Hook(HMENU hMenu, UINT uIDEnableItem, UINT uEnable)
{
    ...
    {
        HWND hwnd = GetTargetWindowAtCursor();
...
static void HandleInitMenuPopup(HWND hWnd, HMENU hPopup)
{
    HWND svWnd = nullptr;
    {
        WCHAR cls[64] = {};
        GetClassNameW(hWnd, cls, 64);
        if (!wcscmp(cls, L"SHELLDLL_DefView") || IsTCWindow(hWnd))
            svWnd = hWnd;
    }
    if (!svWnd)
    {
        ...
        struct CB {
            static BOOL CALLBACK Enum(HWND hw, LPARAM lp) {
                WCHAR cls[64]={};GetClassNameW(hw,cls,64);
                if (!wcscmp(cls,L"SHELLDLL_DefView") || IsTCWindow(hw)){((S*)lp)->found=hw;return FALSE;}
                return TRUE;
            }
        };
                                  ...
                                  WPARAM wParam, LPARAM lParam)
{
    ...
    {
        HWND target = nullptr;
        if (IsShellViewWindow(hWnd) || IsTCWindow(hWnd))
            target = hWnd;
        else if (g_menubarShellView && (IsShellViewWindow(g_menubarShellView) || IsTCWindow(g_menubarShellView)))
            target = g_menubarShellView;
        else if (g_lastContextMenuShellView &&
                 (IsShellViewWindow(g_lastContextMenuShellView) || IsTCWindow(g_lastContextMenuShellView)))
...
int WINAPI TranslateAcceleratorW_Hook(HWND hWnd, HACCEL hAccTable, LPMSG lpMsg)
{
        ...
        !(GetKeyState(VK_MENU)   & 0x8000))
    {
        ...
        if (focusWnd)
        {
            ...
            if (!wcscmp(className, L"Edit") || !wcscmp(className, L"TEdit"))
            {
        ...
        HWND target = nullptr;
        if (IsShellViewWindow(hWnd) || IsTCWindow(hWnd))             target = hWnd;
        else if (IsShellViewWindow(lpMsg->hwnd) || IsTCWindow(lpMsg->hwnd)) target = lpMsg->hwnd;
...
LRESULT WINAPI DispatchMessageW_Hook(const MSG* lpMsg)
{
        ...
        !(GetKeyState(VK_MENU) & 0x8000))
    {
        ...
        HWND target = nullptr;
        if (IsTCWindow(lpMsg->hwnd) || IsShellViewWindow(lpMsg->hwnd)) 
            target = lpMsg->hwnd;

        if (target && CanPasteInWindow(target))
        {
            Wh_Log(L"DispatchMessageW_Hook: Ctrl+V intercepted for hwnd=%p", target);
            if (PerformPaste(target)) return 0;
        }
    }
    return DispatchMessageW_Orig(lpMsg);
}
...
BOOL Wh_ModInit()
{
    ...
    Wh_SetFunctionHook((void*)DispatchMessageW,
                       (void*)DispatchMessageW_Hook,
                       (void**)&DispatchMessageW_Orig);
    return TRUE;
}
